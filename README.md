...
غرفة الترجمة في مجتمع أسس على شبكة متركس: [‪#localization:aosus.org‬](https://matrix.to/#/#localization:aosus.org)،
أو في تلجرام: <https://t.me/aosus> (اختر غرفة الترجمة)،
أو عبر [مسائل GitHub](https://github.com/noureddin/ysmu/issues/).
...
الشعار هو ملفا U+1F304 من [Twemoji](https://twemoji.twitter.com/) برخصة CC-BY 4.0،
مع حرف العين [بالخط الأميري](https://www.amirifont.org/).
واُستعمل [ضاغط نانو من Vecta](https://vecta.io/nano) لتصغير حجم ملفه.
...
- مجلد <code dir="ltr">.p</code>: فيه بُريمج التحويل والمكتبات المساعدة، وهو يحوّل ملفات البيانات إلى صفحات الوب والمعجم المختصر.
- مجلد <code dir="ltr">.t</code>: فيه الأجزاء الكبيرة الثابتة لصفحات الوب الناتجة.
...
- ملف `Ysmark.md`: وصف إنساني للغة التنسيق المستعملة في هذا المشروع. [اقرأه ⬉](https://github.com/noureddin/ysmu/blob/main/Ysmark.md)
...
<p dir="rtl">قد يكون للفظ ترجمات مختلفة حسب المجال، عندئذٍ يُحدد المجال بقوس مجعد مزدوج، مثل <code>{{في أنظمة إدارة النسخ}}</code>. وهذا قد يكون في سطر وحده (أي متبوع بمسافتين) ليعني أن الأسطر التالية حتى آخر الفقرة لهذا المجال، وقد يكون على سطر الترجمة نفسها (أي متبوع بمسافة واحدة ميمية (<code dir="ltr">&amp;emsp;</code>)).

<p dir="rtl">ترجمات المجالات المختلفة تُفصل بثلاث مسافات، لتعامل معاملة الفقرات المستقلة.

<p dir="rtl">ترجمة المجال الواحد قد تكون عدة أسطر مستقلة، عندئذٍ تفصل بمسافتين، ليُجعلا <code dir="ltr">&lt;br&gt;</code> أو ما يكافئها.

<p dir="rtl">وفي المجال الواحد قد توجد معانٍ فرعية (تُحاط بقوسين مجعدين مفردين، مثل `{لأمر خطير}` في مادة alert)، وتعامل معاملة المجالات. وقد تتبعها مسافتين لتكون على سطر وحدها، أو تتبعها مسافة واحدة نونية (<code dir="ltr">&amp;ensp;</code>).
...
لا نستخدم الاختصارات الودية الدارجة، مثل اختصار repository إلى repo.
